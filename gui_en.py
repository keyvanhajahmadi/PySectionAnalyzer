...
class CentroidGUI:
    def __init__(self, root, parent=None):
        ...
        if parent is None:
            self.root.title("Composite Centroid Calculator")
        ...
        self.shapes = {
            ...
            "Quarter Ellipse": {
                "icon": "⬜", "params": [("a", "Semi-axis a"), ("b", "Semi-axis b")],
    ...
    def _build_ui(self):
        self._build_header()
        content = tk.Frame(self.parent, bg=C["mantle"])
    ...
    def _build_header(self):
        h = tk.Frame(self.parent, bg=C["blue"], height=64)
        ...
        tk.Label(h, text="Composite Centroid Calculator", bg=C["blue"], fg=C["base"],
                 font=self.F["title"]).pack(side="left", pady=18)
        tk.Frame(h, bg=C["blue"], width=2).pack(side="left", fill="y", padx=10, pady=14)
        tk.Label(h, text="Created by Keyvan HajAhmadi", bg=C["blue"], fg=C["base"],
                 font=(self.font_family, 10, "italic")).pack(side="right", padx=(4, 16))
        tk.Label(h, text="F11 = Fullscreen  |  Esc = Exit", bg=C["blue"], fg=C["base"],
                 font=self.F["norm"]).pack(side="right", padx=22)

    def _build_input_card(self, parent):
        card = ttk.LabelFrame(parent, text="  📐  Add Component  ", style="Card.TLabelframe")
        card.pack(fill="x", pady=(0, 4))

        top = tk.Frame(card, bg=C["surface0"])
        top.pack(fill="x")

        tk.Label(top, text="Shape:", bg=C["surface0"], fg=C["subtext1"],
        ...
        tk.Label(top, text="Type:", bg=C["surface0"], fg=C["subtext1"],
                 font=self.F["bold"]).grid(row=0, column=2, padx=(20, 6), pady=4, sticky="e")
        self.fill_var = tk.StringVar(value="◆  Solid")
        ttk.Combobox(top, textvariable=self.fill_var,
                     values=["◆  Solid", "○  Void"], state="readonly",
        ...
        ttk.Button(card, text="➕  Add Component", style="Accent.TButton",
    ...
    def _build_canvas_card(self, parent):
        card = ttk.LabelFrame(parent, text="  🎨  Canvas Preview  ", style="Card.TLabelframe")
    ...
    def _build_table_card(self, parent):
        card = ttk.LabelFrame(parent, text="  📊  Numerical Report  ", style="Card.TLabelframe")
        card.pack(side="right", fill="both", expand=True, padx=(6, 0))
        cols = ("Shape", "Parameters", "x̄", "ȳ", "Area", "Type")
    ...
    def _build_result_card(self, parent):
        card = ttk.LabelFrame(parent, text="  📍  Final Result  ", style="Card.TLabelframe")
        card.pack(fill="x", pady=(4, 0))
        inner = tk.Frame(card, bg=C["surface0"])
        inner.pack(fill="x", pady=4)
        self.result_label = tk.Label(inner, text="No components added yet.",
                                     bg=C["surface0"], fg=C["green"],
                                     font=self.F["result"])
        self.result_label.pack(side="left", padx=10)
        ttk.Button(inner, text="🔄  Reset", style="Danger.TButton",
                   command=self._reset).pack(side="right", padx=10)

    def _build_status_bar(self):
        bar = tk.Frame(self.parent, bg=C["surface0"], height=28)
        bar.pack(fill="x", side="bottom")
        bar.pack_propagate(False)
        self.status_label = tk.Label(bar, text="●  Ready", bg=C["surface0"],
    ...
    def _add_part(self):
        ...
        sign = 1 if "Solid" in self.fill_var.get() else -1
        ...
        self._redraw_canvas()
        self._update_result()
        self._update_status(f"●  Component «{shape_name}» added")

    def _update_result(self):
        if abs(self.area_sum) < 1e-12:
            self.result_label.config(text="⚠  Net Area is zero.", fg=C["peach"])
            return
        xc = self.x_sum / self.area_sum
        yc = self.y_sum / self.area_sum
        self.result_label.config(
            text=f"x̄ = {xc:.4f}     ȳ = {yc:.4f}     (Area Net = {self.area_sum:.4f})",
            fg=C["green"],
        )
    ...
    def _reset(self):
        ...
        self.row_index = 0
        self._redraw_canvas()
        self.result_label.config(text="No components added yet.", fg=C["subtext0"])
    ...
    def _draw_shape(self, part):
        ...
        if shape == "Rectangle":
            ...
            c.create_polygon(x1, y1, x2, y2, x3, y3, fill=fill, outline=outline, width=2,
                             stipple="" if s == 1 else "gray12")
        elif shape in ("Quarter Circle", "Semi Circle"):
            r = p["r"] * self.scale
            ox, oy = self._to_canvas(0, 0)
            extent = -90 if shape == "Quarter Circle" else 180
            c.create_arc(ox - r, oy - r, ox + r, oy + r, start=0, extent=extent,
                         fill=fill, outline=outline, width=2,
                         stipple="" if s == 1 else "gray12")
        elif shape in ("Quarter Ellipse", "Semi Ellipse"):
            rx, ry = p["a"] * self.scale, p["b"] * self.scale
            ox, oy = self._to_canvas(0, 0)
            extent = -90 if shape == "Quarter Ellipse" else 180
