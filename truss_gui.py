...
TOOLS = {
    "node":    {"label": "🟢 گره",     "desc": "کلیک کن تا گره جدید بزاری"},
    "member":  {"label": "🔵 عضو",     "desc": "روی دو گره کلیک کن تا عضو وصل شه"},
    "support": {"label": "🟡 تکیه‌گاه","desc": "روی گره کلیک کن تا تکیه‌گاه بزاری"},
    "load":    {"label": "🔴 بار",     "desc": "روی گره کلیک کن تا بار وارد کنی"},
    "dim":     {"label": "📐 اندازه",  "desc": "روی عضو کلیک کن تا ابعادش رو ببینی"},
    "delete":  {"label": "❌ پاک‌کردن","desc": "روی گره یا عضو کلیک کن تا پاک شه"},
    "select":  {"label": "🖱️ انتخاب",  "desc": "گره رو بگیر و جابجا کن"},
}
...
class TrussGUI:
    def __init__(self, parent):
        ...
        self._REF_COLORS = {"سبز": C["green"], "قرمز": C["pink"], "آبی": C["blue"],
                            "زرد": C["yellow"], "نارنجی": C["peach"], "بنفش": C["lavender"]}
    ...
    def build_ui(self):
        ...
        tk.Label(hdr_row, text="تحلیل‌گر سازه‌های خرپایی", bg=C["mantle"], fg=C["text"],
        ...
        tk.Label(hdr_f, text="⚙️  تنظیمات", bg=C["surface0"], fg=C["blue"],
        ...
        tk.Label(res_card, text="📊  نتایج اعضا", bg=C["surface0"], fg=C["blue"],
        ...
        tk.Label(rx_card, text="📍  واکنش‌ها", bg=C["surface0"], fg=C["blue"],
                 font=self.F["big"]).pack(anchor="w", padx=10, pady=(8, 2))
        self.react_label = tk.Label(rx_card, text="هنوز حل نشده.", bg=C["surface0"],
        ...
        self.status_label = tk.Label(sb, text="آماده", bg=C["surface0"], fg=C["overlay0"],
    ...
    def _build_prop_node(self):
        ...
        btn = self._make_btn(self.prop_content, "➕ افزودن گره", C["blue"], C["lavender"],
                             self._add_node_keyboard)
        btn.pack(pady=4)
        tk.Label(self.prop_content, text="یا روی صفحه کلیک کن", bg=C["surface0"],
                 fg=C["subtext0"], font=self.F["norm"]).pack(pady=(0, 2))
        self.status("مختصات رو تایپ کن و Enter بزن یا روی صفحه کلیک کن.")
    ...
    def _add_node_keyboard(self):
        try:
            ...
            messagebox.showerror("خطا", "X و Y باید عدد باشن.")
            return
        self.nodes.append((x, y))
        self.solved = False
        self.status(f"گره {len(self.nodes)-1}: ({x:.2f}, {y:.2f})")
    ...
    def _build_prop_member(self):
        self._clear_prop()
        tk.Label(self.prop_content, text="گره اول رو انتخاب کن...", bg=C["surface0"],
                 fg=C["subtext0"], font=self.F["norm"]).pack()
        if self.selected_node is not None:
            self.status(f"گره {self.selected_node} انتخاب شد. حالا گره دوم رو کلیک کن.")

    def _build_prop_support(self):
        self._clear_prop()
        tk.Label(self.prop_content, text="نوع تکیه‌گاه:", bg=C["surface0"],
                 fg=C["text"], font=self.F["bold"]).pack(anchor="w")
        self.sup_var = tk.StringVar(value="مفصلی")
        ttk.Combobox(self.prop_content, textvariable=self.sup_var,
                      values=["مفصلی (Rx,Ry)", "گیردار (Rx,Ry,M)", "غلتک قائم (Ry)", "غلتک افقی (Rx)", "طنابی (Rx,Ry)"],
                      state="readonly", width=22, font=self.F["norm"]).pack(pady=4)
        tk.Label(self.prop_content, text="روی گره کلیک کن.", bg=C["surface0"],
                 fg=C["subtext0"], font=self.F["norm"]).pack()
        self.status("روی گره کلیک کن تا تکیه‌گاه بزاری.")

    def _build_prop_load(self):
        self._clear_prop()
        tk.Label(self.prop_content, text="مقدار بار:", bg=C["surface0"],
                 fg=C["text"], font=self.F["bold"]).pack(anchor="w")
        frow = tk.Frame(self.prop_content, bg=C["surface0"])
        frow.pack(pady=2)
        ...
        tk.Label(self.prop_content, text="روی گره کلیک کن.", bg=C["surface0"],
                 fg=C["subtext0"], font=self.F["norm"]).pack()
        self.status("روی گره کلیک کن تا بار وارد کنی.")
    ...
    def set_tool(self, tool_id):
        ...
        if tool_id == "node":
            ...
            self._clear_prop()
            tk.Label(self.prop_content, text="روی یک عضو کلیک کن\nتا ابعادش نشون داده بشه.", bg=C["surface0"],
                     fg=C["subtext0"], font=self.F["norm"]).pack()
            self.status("روی عضوی کلیک کن تا Δx, Δy, L رو ببینی.")
        elif tool_id == "delete":
            self._clear_prop()
            tk.Label(self.prop_content, text="روی گره یا عضو کلیک کن.", bg=C["surface0"],
                     fg=C["subtext0"], font=self.F["norm"]).pack()
            self.status("روی گره یا عضو کلیک کن تا پاک بشه.")
        elif tool_id == "select":
    ...
    def _build_prop_select(self):
        self._recalc_limits()
        self._clear_prop()
        tk.Label(self.prop_content, text="تنظیمات محورها (متر):", bg=C["surface0"],
                 fg=C["subtext1"], font=self.F["bold"]).pack(pady=(0, 4))
        fr = tk.Frame(self.prop_content, bg=C["surface0"])
        fr.pack()
        tk.Label(fr, text="X از:", bg=C["surface0"], fg=C["subtext1"], font=self.F["norm"]).pack(side="left")
        ...
        tk.Label(fr, text=" تا:", bg=C["surface0"], fg=C["subtext1"], font=self.F["norm"]).pack(side="left")
        ...
        fr2.pack()
        tk.Label(fr2, text="Y از:", bg=C["surface0"], fg=C["subtext1"], font=self.F["norm"]).pack(side="left")
        ...
        tk.Label(fr2, text=" تا:", bg=C["surface0"], fg=C["subtext1"], font=self.F["norm"]).pack(side="left")
        ...
        fr3.pack(pady=2)
        tk.Label(fr3, text="گام:", bg=C["surface0"], fg=C["subtext1"], font=self.F["norm"]).pack(side="left")
        ...
        tk.Label(fr3, text="متر", bg=C["surface0"], fg=C["subtext1"], font=self.F["norm"]).pack(side="left", padx=2)
        self._make_btn(self.prop_content, "✓ اعمال", C["green"], C["teal"],
        ...
        self.status("محدوده محورها و گام grid رو با کیبورد تنظیم کن.")

        ttk.Separator(self.prop_content, orient="horizontal").pack(fill="x", pady=6)
        tk.Label(self.prop_content, text="وزن اعضا:", bg=C["surface0"], fg=C["subtext1"],
                 font=self.F["bold"]).pack()
        fw = tk.Frame(self.prop_content, bg=C["surface0"])
        fw.pack(pady=2)
        tk.Label(fw, text="وزن مخصوص خطی (kN/m):", bg=C["surface0"], fg=C["subtext1"],
        ...
        self._make_btn(self.prop_content, "⚖️ توزیع وزن به گره‌ها", C["sapphire"], C["teal"],
                       self._apply_self_weight).pack(pady=2)

        ttk.Separator(self.prop_content, orient="horizontal").pack(fill="x", pady=6)
        tk.Label(self.prop_content, text="سطوح مبنا:", bg=C["surface0"], fg=C["subtext1"],
                 font=self.F["bold"]).pack()
        ...
        tk.Label(frl, text="برچسب:", bg=C["surface0"], fg=C["subtext1"],
        ...
        self.entry_ref_label.insert(0, "زمین")
        tk.Label(frl, text="رنگ:", bg=C["surface0"], fg=C["subtext1"],
                 font=self.F["norm"]).pack(side="left", padx=(6, 0))
        self.ref_color_var = tk.StringVar(value="سبز")
        ttk.Combobox(frl, textvariable=self.ref_color_var,
                      values=["سبز", "قرمز", "آبی", "زرد", "نارنجی", "بنفش"],
                      state="readonly", width=7, font=self.F["norm"]).pack(side="left", padx=1)
        self._make_btn(self.prop_content, "➕ افزودن خط مبنا", C["teal"], C["green"],
    ...
    def _apply_view(self):
        try:
            ...
            messagebox.showerror("خطا", "همه مقادیر باید عدد باشن.")
            return
        if xmin >= xmax or ymin >= ymax:
            messagebox.showerror("خطا", "min باید کوچک‌تر از max باشه.")
            return
        if grid <= 0:
            messagebox.showerror("خطا", "گام باید مثبت باشه.")
            return
        ...
        self._update_transform()
        self.draw()
        self.status(f"محورها: X[{xmin}, {xmax}]  Y[{ymin}, {ymax}]  گام={grid}")

    def _add_ref_line(self):
        try:
            y = float(self.entry_ref_y.get())
        except ValueError:
            messagebox.showerror("خطا", "Y باید عدد باشه.")
            return
        ...
        self._rebuild_ref_list()
        self.draw()
        self.status(f"خط مبنا '{label}' در Y={y:.2f} اضافه شد.")
    ...
    def _rebuild_ref_list(self):
        ...
        if not self.ref_lines:
            tk.Label(self.ref_list_frame, text="(هیچ)", bg=C["surface0"],
                     fg=C["subtext0"], font=self.F["norm"]).pack()
            return
    ...
    def _apply_self_weight(self):
        if not self.members:
            self.status("هیچ عضوی تعریف نشده.")
            return
        try:
            w = float(self.entry_selfw.get())
        except ValueError:
            messagebox.showerror("خطا", "وزن مخصوص باید عدد باشه.")
            return
        if w <= 0:
            self.status("وزن صفر — هیچ باری اضافه نشد.")
            return
        node_loads = {}
        ...
        self.status(f"وزن اعضا توزیع شد. مجموع بار قائم اضافه‌شده: {total:.2f} kN")
        self.draw()
    ...
    def _apply_dim(self, mk, i, j):
        try:
            ...
            messagebox.showerror("خطا", "L و زاویه باید عدد باشن.")
            return
        if new_L <= 0:
            messagebox.showerror("خطا", "طول باید مثبت باشه.")
            return
        ...
        self.solved = False
        self.status(f"عضو {mk} به L={new_L:.3f}, زاویه={new_ang:.1f}° تغییر کرد.")
        self.draw()
    ...
    def on_click(self, e):
        if self.current_tool == "node":
            ...
            if self.selected_node is None:
                ...
                self.status(f"گره اول {ni} انتخاب شد. گره دوم رو کلیک کن.")
                self.draw()
            else:
                if ni != self.selected_node:
                    self.members.append((self.selected_node, ni))
                    self.solved = False
                    self.status(f"عضو {self.selected_node}-{ni} اضافه شد.")
                self.selected_node = None
                self.draw()
            ...
            if ni is None:
                return
            sup_map = {"مفصلی (Rx,Ry)": "pinned", "گیردار (Rx,Ry,M)": "fixed",
                       "غلتک قائم (Ry)": "roller", "غلتک افقی (Rx)": "roller_x",
                       "طنابی (Rx,Ry)": "cable"}
            ...
            if idx is not None:
                ...
                self.status(f"تکیه‌گاه گره {ni} به {rtype} تغییر کرد.")
            else:
                self.supports.append((ni, rtype))
                self.status(f"تکیه‌گاه {rtype} به گره {ni} اضافه شد.")
            self.solved = False
            self.draw()
            ...
            try:
                ...
                messagebox.showerror("خطا", "Fx و Fy باید عدد باشن.")
                return
            if abs(fx) < 1e-12 and abs(fy) < 1e-12:
                self.status("بار صفر وارد نشد.")
                return
            idx = None
            ...
            if idx is not None:
                ...
                self.status(f"بار گره {ni} به ({fx}, {fy}) تغییر کرد.")
            else:
                self.loads.append((ni, fx, fy))
                self.status(f"بار ({fx}, {fy}) به گره {ni} اضافه شد.")
            self.solved = False
            self.draw()
            ...
            if mk is not None:
                ...
                self._clear_prop()
                hdr = tk.Label(self.prop_content, text=f"ویرایش عضو {mk}  ({i}→{j})",
                               bg=C["surface0"], fg=C["peach"], font=self.F["bold"])
                hdr.pack(pady=(0, 4))
                fr_l = tk.Frame(self.prop_content, bg=C["surface0"])
                fr_l.pack()
                tk.Label(fr_l, text="L (متر):", bg=C["surface0"], fg=C["subtext1"],
                         font=self.F["norm"]).pack(side="left")
                ...
                tk.Label(fr_l, text="زاویه°:", bg=C["surface0"], fg=C["subtext1"],
                         font=self.F["norm"]).pack(side="left", padx=(6, 0))
                ...
                self._make_btn(self.prop_content, "✓ اعمال تغییرات", C["green"], C["teal"],
    ...
    def on_release(self, e):
        if self.drag_node is not None:
            self.status(f"گره {self.drag_node} جابجا شد.")
        self.drag_node = None
    ...
    def solve(self):
        if len(self.nodes) < 2:
            messagebox.showwarning("هشدار", "حداقل ۲ گره نیازه.")
            return
        if not self.members:
            messagebox.showwarning("هشدار", "هیچ عضوی تعریف نشده.")
            return
        if not self.supports:
            messagebox.showwarning("هشدار", "حداقل یه تکیه‌گاه بزار.")
            return
        try:
            forces, reactions = truss.solve_truss(self.nodes, self.members, self.supports, self.loads)
        except Exception as e:
            messagebox.showerror("خطا", f"خرپا قابل حل نیست:\n{str(e)}")
            return
        ...
        for k, (i, j) in enumerate(self.members):
            ...
            if abs(f) < 1e-6:
                ...
                typ, tag = "کششی ✓", "tension"
            else:
                typ, tag = "فشاری ✗", "compression"
        ...
        self.status(f"✅ حل شد. {len(forces)} عضو")
        self.draw()
    ...
    def reset_all(self):
        ...
        for item in self.tree.get_children():
            self.tree.delete(item)
        self.react_label.config(text="پاک شد.", fg=C["subtext0"])
        self.canvas.delete("all")
        self.status("🔄 همه چیز پاک شد.")
